 ...
 * file that ships to the browser — Hugo is a static-site generator, so the
 * shipped artifact is a public SPA: PKCE only.
 *
 * Hugo has no build step that injects secrets, so this file ships placeholders
 * and a tiny config-loading shim. For a real deployment you would either inline
 * the values via a Hugo `data/` JSON file rendered server-side at build, or
 * fetch a non-secret config JSON from a known path on the same origin at boot.
 */
  ...
  async function signIn() {
    if (config.clientId === "CONFIGURE_ME") {
      console.warn(
        ...
        "Fill in CONFIGURE_ME values in static/js/auth.js — see /docs/AUTH.md."
      );
      return;
    }
